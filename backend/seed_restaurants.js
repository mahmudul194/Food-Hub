...
async function seed() {
    try {
        ...
        for (const category of categories) {
            ...
            for (const r of restaurants) {
                ...
                const [resResult] = await db.query(
                    'INSERT INTO Restaurants (name, rating, delivery_time, image_url, description) VALUES (?, ?, ?, ?, ?)',
                    [r.name, rating, delivery_time, r.image, `${r.desc} - ${category}`]
                );

                const resId = resResult.insertId;

                // Insert 3 dummy items for each restaurant to keep it populated
                await db.query('INSERT INTO MenuItems (restaurant_id, name, description, price, category, image_url) VALUES (?, ?, ?, ?, ?, ?)', [
        ...
        console.log('Seeding incredibly successful!');
        process.exit(0);
    } catch (e) {
